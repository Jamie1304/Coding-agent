    ...
    try {
      switch (call.function.name) {
        ...
        case "replace_text": {
          ...
          return toolSuccess(
            { path, replacements, operation: "replace" },
...
function buildTools(sandbox: SendTurnInput["sandbox"]): Array<Record<string, unknown>> {
  ...
  if (sandbox === "workspace-write") {
    tools.push(
      ...
      functionTool("delete_file", "Delete one regular workspace file. Directories cannot be deleted.", {
        type: "object",
...
async function replaceWorkspaceText(
  ...
  if (!replaceAll && occurrences !== 1) {
    throw new Error(`oldText matched ${String(occurrences)} times; use replaceAll or a larger exact block`);
  }
...
function normalizeCommandName(command: string): string {
  return basename(command).toLocaleLowerCase().replace(/\.(?:cmd|exe)$/u, "");
}
